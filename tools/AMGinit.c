...
void AMGINIT(CSRMAT *A, ILUPACKPARAM *param)
{
  /*
    ...
    ipar        integer parameters ipar[0,...,33]
		...
		ipar[7]     decide which kind of scaling should be combined
			    ...
			    Bit 0-2 initial preprocessing
                            ...
                            Bit 2    left/right scaling first  0/  4
			    default: 3 (l+r, l first)
		...
		ipar[16,...,19] currently unused
                ...
                ipar[20,...35] parameters used by SPARSKIT solvers
		...
		ipar[26,...35] further parameters used by SPARSKIT solvers
		            default: 0
    ...
    fpar        floating point parameters 
		...
		fpar[7]     drop tolerance for ILUTP (ILDLC), used relative to 
                            fpar[0].	    
			    default: sqrt(eps)
                ...
                fpar[20,...,35] floating parameters required for SPARSKIT 
		            solvers
		...
		fpar[21]    absolute error tolerance
		            default: 0
			    fpar[22]=
		ipar[23,...35] further parameters used by SPARSKIT solvers
		            default: 0
   ...
   */
   ...
   // write local copies to their global counter parts
   // write_globals(ILUPACK_secnds_loc, ILUPACK_mem_loc);
   ...
   */
   ...
   param->nthreads=1.0; //omp_get_num_procs();
   param->loadbalancefactor=1.0;
   // make sure that not too many leaves are requested
   // CONSISTENT_THREADS(*A, param);
