/* $Id: cosine_blocks.c 3620 2017-09-09 15:19:03Z bolle $ 
...
void COSINE_BLOCKS(SPARSEMATRIX *A,
		   integer *p, integer *invq,
		   integer *blocksize, integer *nblocks,
		   REALS tau)
{  
  /*
    cosine-based compression
  ...
  *nblocks=-1;
  for (i=0; i<n; i++) {
      // if i-th row is not yet assigned to any block
      if (group[i]<0) {            
	 // start a new block
	 blocksize[++(*nblocks)]=1;
	 // store permutation
