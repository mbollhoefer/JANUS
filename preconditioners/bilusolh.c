/* $Id: bilusolh.c 4175 2018-04-29 21:01:46Z bolle $
...
void BILUSOLH(SPARSEBLOCKMATRIX *BL, SPARSEBLOCKMATRIX *BiD, SPARSEBLOCKMATRIX *BUT, integer *pivots,
	      FLOAT *rhssol, FLOAT *dbuff, integer n, integer m)
{  
  /*
    solve linear system(s) with conjugate transposed block incomplete LDU factorization
  ...
  */
  ...
  integer i,j,k,l,r,s,tt,i2, // counters
  ...
  // --------------------------------------------------------------------------
  // forward substitution with conj(BUT) instead of BL
  ...
  for (i=0; i<BiD->nblocks; i++,psol+=k,tt+=k) {
	 ...
	 if (k==1) {
	    ...
	    beta = 1.0;
#else
	    // conjugate buff and sol in advance and after the update
	    // conj(buff) = -1*BUT21*conj(sol) + 1*conj(buff)
	    pb=buff;
	    for (r=0; r<l; r++,pb++)
	        pb->i=-pb->i;
	    for (r=0; r<k; r++)
...
#if !defined _SINGLE_REAL_ && !defined _DOUBLE_REAL_
	    // conjugate buff and sol in advance and after the update
	    // buff = conj(-1*     BUT21 *conj(sol) + 1*conj(buff))
	    //      =      -1*conj(BUT21)*     sol  + 1*     buff
	    pb=buff;
	    for (r=0; r<l; r++,pb++)
	        pb->i=-pb->i;
	    for (r=0; r<k; r++)
  ...
  // --------------------------------------------------------------------------
  // diagonal block multiplication with the conjugate transposed diagonal blocks
	 ...
	 if (k==1) {
	    ...
	    *psol*=*pD;
	    ...
	    if (invert_blocks) {
	       ...
	       GETRS("c", &k, &m, pD,&k, pivots+tt, psol,&n, &ierr,1);
	       if (ierr<0) {
...
#else
	    CBLAS_DOTI(l, pU, pi, psol-tt, &val);
