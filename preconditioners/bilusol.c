/* $Id: bilusol.c 4175 2018-04-29 21:01:46Z bolle $
...
void BILUSOL(SPARSEBLOCKMATRIX *BL, SPARSEBLOCKMATRIX *BiD, SPARSEBLOCKMATRIX *BUT, integer *pivots,
	     FLOAT *rhssol, FLOAT *dbuff, integer n, integer m)
{  
  /*
    solve linear system(s) with block incomplete LDU factorization
	 ...
	 if (k==1) {
	    ...
	    *psol*=*pD;
	    ...
	    if (invert_blocks) {
	       ...
	       GETRS("n", &k, &m, pD,&k, pivots+tt, psol,&n, &ierr,1);
	       if (ierr<0) {
