 ...
 */
integer PERMRCM(CSRMAT A, FLOAT *prowscale, FLOAT *pcolscale,
 ...
 */
  ...
  integer i,j,k,l,bnd,bndt,scale,nrm,ierr=0, iflag;
...
#include "scaleprefix.c"
   ...
   for (i=0; i<A.nc; i++) {
     if (A.ia[p[i]]-A.ia[p[i]-1]>bnd || param->ibuff[p[i]-1]>bndt) {
        invq[k]=p[i];
        k++;
     }
     else {
