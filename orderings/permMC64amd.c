 ...
 */
integer PERMMC64AMD(CSRMAT A, FLOAT *proscal, FLOAT *pcoscal, integer *p, integer *invq, integer *nB, 
		ILUPACKPARAM *param)
{
/*
 ...
 */
    ...
    */
    ...
    /* scale matrix such that the maximum entry in each column/row is one in
       absolute value */
    nz=A.ia[A.nc]-1;
    // cperm=(integer *) MALLOC((size_t)n*sizeof(integer),"permMC64_amd:cperm");
    liw=5*n;
    if (liw<=param->niaux)
       iw=param->iaux;
    else
       iw=(integer *) MALLOC((size_t)liw*sizeof(integer),"permMC64_amd:iw");
    ldw=3*n+nz;
    if (ldw<=param->ndaux)
       dw=(REALS *)param->daux;
    else
       dw=(REALS *)MALLOC((size_t)ldw*sizeof(FLOAT),"permMC64_amd:dw");
    b=dw+3*n;
    ...
    if (flags&SYMMETRIC_STRUCTURE) {
	  ...
	  C.ja=(integer *)MALLOC((size_t)nz*sizeof(integer), "permMC64_amd:C.ja");
       C.ia[0]=1;
       for (i=0; i<A.nc; i++) {
	   ii=p[i]-1;
	   k=C.ia[i]-1;
	  ...
	  for (i=0; i<iflag; i++) {
       ...
       ierr=amd_l_order(B.nc,B.ia,B.ja,cperm,Control,Info);
#else
       amd_defaults(Control);
       ierr=amd_order(B.nc,B.ia,B.ja,cperm,Control,Info);
#endif
       if (!iflag)
	  B.ja=FREE(B.ja);

       // convert permutation back to FORTRAN style
       for (i=0; i<B.nc; i++) {
	   cperm[i]++;
       } // end for
	  ...
	  iflag=B.ia[B.nc]-1; 
	   ...
	   j=B.ia[i]-1;
	   k=B.ia[i+1]-1-j;
	   QQSORTI(B.ja+j,param->ibuff+A.nc,&k);
       }
...
#ifdef _LONG_INTEGER_
       amd_l_defaults(Control);
       ierr=amd_l_order(A.nc,B.ia,B.ja,cperm,Control,Info);
#else
       amd_defaults(Control);

       ierr=amd_order(A.nc,B.ia,B.ja,cperm,Control,Info);
#endif
       if (!iflag)
	  B.ja=FREE(B.ja);
