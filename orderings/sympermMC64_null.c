...
#define MYSYMPERMMC64NULL    DSSMperm_mc64_null

#else
#define MYSYMMWM             SSSMsmwm
#define MYSYMPERMMC64NULL    SSSMperm_mc64_null
#endif
...
#define MYSYMPERMMC64NULL    CSSMperm_mc64_null
#else
#define MYSYMMWM             ZSSMsmwm
#define MYSYMPERMMC64NULL    ZSSMperm_mc64_null
#endif
...
#define MYSYMPERMMC64NULL   CSHRperm_mc64_null
#else
#define MYSYMMWM            ZSHRsmwm
#define MYSYMPERMMC64NULL   ZSHRperm_mc64_null
#endif
...
/*
 ...
 here 1) maximum weight matching (MC64)
      2) associated symmetric block partitioning and block reordering by
 ...
 */
integer MYSYMPERMMC64NULL(CSRMAT A, FLOAT *prowscale, FLOAT *pcolscale, 
 ...
 */ 
    ...
    // mexPrintf("entering SYMPERMMC64NULL\n");fflush(stdout);
    ierr=0;
...
#include "scaleprefix.c"
    ...
    if (param->flags&BLOCK_STRUCTURE) {
       ...
       for (i=0; i<k; i++) {
	   if (invq[i+1]<=0) {
	      printf("sympermmc64_null: block structure is incorrect for block %ld\n",i+1);
	      exit(0);
	   }
	   invq[i+1]+=invq[i];
       }
	  ...
	  C.ja=(integer *)MALLOC((size_t)nz*sizeof(integer), "sympermmc64_null:C.ja");
