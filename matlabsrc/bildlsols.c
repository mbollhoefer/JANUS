/* $Id: bildlsols.c 4169 2018-04-26 16:34:10Z bolle $ */
/* ========================================================================== */
/* === bildlsols mexFunction ============================================== */
/* ========================================================================== */

/*
    Usage:

    Given a SYMMETRIC block factorization BL BiD^{-1} BL^T, solve a linear system

    Example:

    z=bildlsols(BL,BiD,pivots,y)
...
)
{
    ...
    if (!mxIsCell(BL_input)) {
       mexErrMsgTxt("bildlsols: first input matrix must be in cell format.") ;
    }
    ...
    if (!mxIsCell(BiD_input)) {
       mexErrMsgTxt("bildlsols: second input matrix must be in cell format.") ;
    }
    
    /* The fourth input must be the r.h.s. */
    y_input=(mxArray *)prhs[3];
    if (!mxIsNumeric(y_input))
       mexErrMsgTxt("bildlsols: fourth input argument must be numeric.") ;
    n=mxGetM(y_input);
    m=mxGetN(y_input);
    ...
    if (!mxIsNumeric(y_input))
       mexErrMsgTxt("bildlsols: third input argument must be numeric.");
...
#endif
       /* solve triangular system */
       ZSYMbilusol(&BLC,&BiDC,&BLC,pivots, rhssolc,NULL,n,m);
       if (npivots>0)
	  free(pivots);
#ifdef PRINT_INFO
