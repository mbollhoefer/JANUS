...
)
{
   ...
   mwIndex        *P_ja,          /* row indices of input matrix P         */
    ...
    n=*mxGetPr(tmp);
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: input parameter PREC.n imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: input parameter PREC.BL imported\n");fflush(stdout);
#endif
    ZBL.nr=ZBL.nc=n;
    ZBL.nblocks=nblocks;
    ZBL.isreal=0;
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: PREC.BL prepared\n");fflush(stdout);
#endif
    for (j=0; j<nblocks; j++) {
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: input parameter PREC.BiD imported\n");fflush(stdout);
#endif
    ZBiD.nr=ZBiD.nc=n;
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: input parameter PREC.P imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: input parameter PREC.SL imported\n");fflush(stdout);
#endif

    ZHERselbinv(&iA,&ZBL,&ZBiD,NULL,
		S_valuesR,NULL,
		p,invq);
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: symselbinv completed\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: iA exported\n");fflush(stdout);
#endif
