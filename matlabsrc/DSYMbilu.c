/* $Id: DSYMbilu.c 7315 2021-05-28 21:00:20Z bolle $ */
/* ========================================================================== */
/* === DSYMbilu mexFunction ================================================= */
/* ========================================================================== */
...
/*
    ...
    [BL,BiD,P,S_L,determinant,pivots,options]=DSYMbilu(A,options)
	...
	March 9, 2017. JANUS Block ILU R1.0.  
...
)
{
  ...
  timeBegin=time_total=omp_get_wtime();
    ...
    if (!mxIsSparse(A_input)) {
       mexErrMsgTxt("DSYMbilu: input matrix must be in sparse format.") ;
    }
    n=mrows;
#ifdef PRINT_CHECK
...
#ifdef PRINT_INFO
    mexPrintf("DSYMbilu: sparse matrix imported\n");fflush(stdout);
#endif
	...
	if (classIDflags[ifield]==mxCHAR_CLASS) {
	   ...
	   if (!strcmp("matching",fnames[ifield])) {
		 ...
		 invert_blocks=0;
	      ...
	      if (cnt!=n) {
		 mexErrMsgTxt("DSYMbilu: options.blocksize must exactly match the size of the matrix") ;
	      }
	      cnt=0;
	   }
	   else {
...
#ifdef PRINT_INFO
    mexPrintf("DSYMbilu: options structure imported\n");fflush(stdout);
    mexPrintf("DSYMbilu: cosine=%ld, ilu1t=%ld, progressive_aggregation=%ld\n",
    ...
    if (options.nblocks==n||options.nblocks==0||options.blocksize==NULL) {
	  ...
	  ierr=DSYMperm_mc64_null(A, prowscale,pcolscale, p,invq, &nB, &options);
	  ...
	  */
	  ...
	  for (i=0; i<n; i++) {
	      if (p[i]<0 || p[i]>=n) {
		 mexPrintf("DSYMbilu ERROR, wrong permutation, step %ld, p[%ld]=%ld\n",i,i,p[i]);
		 fflush(stdout);
	      }
	      if (invq[i]<0 || invq[i]>=n) {
		 mexPrintf("DSYMbilu ERROR, wrong permutation, step %ld, invq[%ld]=%ld\n",i,i,invq[i]);
		 fflush(stdout);
	      }
	  }
	  for (i=0; i<n; i++)
	  ...
	  for (i=0; i<n; i++) {
	      if (check_buff[p[i]]) {
	         mexPrintf("DSYMbilu ERROR, duplicate index, step %ld, p[%ld]=%ld\n",i,i,p[i]);
		 fflush(stdout);
	      }
	      check_buff[p[i]]=-1;
	  }
	  ...
	  for (i=0; i<n; i++) {
	      if (check_buff[invq[i]]) {
	         mexPrintf("DSYMbilu ERROR, duplicate index, step %ld, invq[%ld]=%ld\n",i,i,invq[i]);
		 fflush(stdout);
	      }
	      check_buff[invq[i]]=-1;
	  }
	  ...
	  for (i=0; i<n; i++) {
	      if (invq[p[i]]!=i) {
	         mexPrintf("DSYMbilu ERROR, inverse permutation does not match, step %ld, p[%ld]=%ld, invq[p[%ld]]=%ld\n",i,i,p[i],i,invq[p[i]]);
		 fflush(stdout);
	      }
	  }
#endif
	  ...
	  */
	  ...
	  for (j=0; j<n; j++) {
	      ...
	      C.val[j]   =(double *) malloc((size_t)C.ncol[j]*sizeof(double));
...
#ifdef PRINT_INFO
	  /* DPrintMatrix(C); */
	  mexPrintf("DSYMbilu: permuted sparse matrix data structures set up\n");fflush(stdout);
#endif
	  ...
	  */
	  ...
	  for (j=0; j<nB; j++) {
	      ...
	      C.val[j]   =(double *) realloc(C.val[j],   (size_t)k*sizeof(double));
	  } /* end for j */
	  ...
	  for (m=nB; j<n; m++,j++) {
	      ...
	      C.val[m]   =(double *) realloc(C.val[m],   (size_t)k*sizeof(double));
	  } /* end for m */
	  C.nr=C.nc=m;
...
#ifdef PRINT_INFO
	  mexPrintf("DSYMbilu: companion matrix for cosine-based algorithm computed\n");fflush(stdout);
#endif
	  ...
	  for (j=0; j<m; j++)
	      p2[j]=invq[j]=j;
	  Dmcosine_sblocks(&C, p2,invq, myblocksize, &mynblocks, TAU);
#ifdef PRINT_INFO
	  mexPrintf("DSYMbilu: cosine applied\n");fflush(stdout);
#endif
#ifdef PRINT_CHECK
	  for (i=0; i<m; i++) {
	      if (p2[i]<0 || p2[i]>=m) {
		 mexPrintf("DSYMbilu ERROR, wrong permutation, step %ld, p2[%ld]=%ld\n",i,i,p2[i]);
		 fflush(stdout);
	      }
	  ...
	  for (i=0; i<m; i++) {
	      if (check_buff[p2[i]]) {
	         mexPrintf("DSYMbilu ERROR, duplicate index, step %ld, p2[%ld]=%ld\n",i,i,p2[i]);
		 fflush(stdout);
	      }
	      check_buff[p2[i]]=-1;
	  }
	  ...
	  for (i=0; i<m; i++) {
	      if (invq[p2[i]]!=i) {
	         mexPrintf("DSYMbilu ERROR, inverse permutation does not match, step %ld, p2[%ld]=%ld, invq[p2[%ld]]=%ld\n",i,i,p2[i],i,invq[p2[i]]);
		 fflush(stdout);
	      }
	  }
#endif
	  ...
	  for (i=0;i<n; i++) {
	      if (invq[i]<0 || invq[i]>=n) {
		 mexPrintf("DSYMbilu ERROR, wrong expanded permutation, step %ld, invq[%ld]=%ld\n", i,i,invq[i]);
		 fflush(stdout);
	      }
	  }
	  for (i=0; i<n; i++)
	  ...
	  */
	  ...
	  for (i=0;i<n; i++) {
	      if (p[i]<0 || p[i]>=n) {
		 mexPrintf("DSYMbilu ERROR, wrong permutation product, step %ld, p[%ld]=%ld\n", i,i,p[i]);
		 fflush(stdout);
	      }
	      if (invq[i]<0 || invq[i]>=n) {
		 mexPrintf("DSYMbilu ERROR, wrong permutation product, step %ld, invq[%ld]=%ld\n", i,i,invq[i]);
		 fflush(stdout);
	      }
	  }
	  for (i=0; i<n; i++)
	  ...
	  free(p2);
#ifdef PRINT_INFO
	  mexPrintf("DSYMbilu: permutation after cosine expanded to incorporate 2x2 pivots\n");fflush(stdout);
#endif
#ifdef _PROFILING_
...
#ifdef PRINT_INFO
	  mexPrintf("DSYMbilu: proper sparse matrix data structures based on A set up\n");fflush(stdout);
#endif
#ifdef _PROFILING_
	  time_pre_processing+=omp_get_wtime()-timeBegin;
#endif
	  ...
	  */
	  ...
	  if (!strcmp("mtmetis",options.ordering)) {
	     ...
	     ierr=DSYMperm_mc64_mtmetis(A, prowscale,pcolscale, p,invq, &n, &options);
#else
	     ierr=DSYMperm_matching_mtmetis(A, prowscale,pcolscale, p,invq, &n, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("DSYMbilu: matching+mtmetis performed\n");fflush(stdout);
#endif
	  }
#ifdef _USE_METIS4_
	  else if (!strcmp("metisn",options.ordering)) { /* METIS nested dissection by nodes */
#ifdef _MC64_MATCHING_
	     ierr=DSYMperm_mc64_metis_n(A, prowscale,pcolscale, p,invq, &n, &options);
#else
	     ierr=DSYMperm_matching_metis_n(A, prowscale,pcolscale, p,invq, &n, &options);
#endif	     
#ifdef PRINT_INFO
	     mexPrintf("DSYMbilu: matching+metisn performed\n");fflush(stdout);
#endif
	  }
	  else if (!strcmp("metise",options.ordering)) {
#ifdef _MC64_MATCHING_
	     ierr=DSYMperm_mc64_metis_e(A, prowscale,pcolscale, p,invq, &n, &options);
#else
	     ierr=DSYMperm_matching_metis_e(A, prowscale,pcolscale, p,invq, &n, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("DSYMbilu: matching+metise performed\n");fflush(stdout);
#endif
	  }
#endif // _USE_METIS4_
	  else if (!strcmp("amd",options.ordering)) {
#ifdef _MC64_MATCHING_
	     ierr=DSYMperm_mc64_amd(A, prowscale,pcolscale, p,invq, &n, &options);
#else
             ierr=DSYMperm_matching_amd(A, prowscale,pcolscale, p,invq, &n, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("DSYMbilu: matching+amd performed\n");fflush(stdout);
#endif
	  }
	  else {
#ifdef _MC64_MATCHING_
	     ierr=DSYMperm_mc64_rcm(A, prowscale,pcolscale, p,invq, &n, &options);
#else
	     ierr=DSYMperm_matching_rcm(A, prowscale,pcolscale, p,invq, &n, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("DSYMbilu: matching+rcm performed\n");fflush(stdout);
#endif
	  }
	  /* switch back to C-style */
	  for (i=0; i<n; i++) {
	  ...
	  ierr=DSYMperm_null(A, prowscale,pcolscale, p,invq, &n, &options);
	  /* switch back to C-style */
	  for (i=0; i<n; i++) {
	      p[i]--; invq[i]--;
	  }
	  ...
	  */
	  ...
	  if (!strcmp("mtmetis",options.ordering)) {
	     ierr=DSYMperm_mtmetis(A, prowscale,pcolscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("DSYMbilu: mtmetis performed\n");fflush(stdout);
#endif
	  }
#ifdef _USE_METIS4_
	  else if (!strcmp("metisn",options.ordering)) {
	     ierr=DSYMperm_metis_n(A, prowscale,pcolscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("DSYMbilu: metisn performed\n");fflush(stdout);
#endif
	  }
	  else if (!strcmp("metise",options.ordering)) {
	     ierr=DSYMperm_metis_e(A, prowscale,pcolscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("DSYMbilu: metise performed\n");fflush(stdout);
#endif
	  }
#endif // _USE_METIS4_
	  else if (!strcmp("amd",options.ordering)) {
	     ierr=DSYMperm_amd(A, prowscale,pcolscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("DSYMbilu: amd performed\n");fflush(stdout);
#endif
	  }
	  else if (!strcmp("rcm",options.ordering)) {
	     ierr=DSYMperm_rcm(A, prowscale,pcolscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("DSYMbilu: rcm performed\n");fflush(stdout);
#endif
	  }
	  else { /* none */
	     ierr=DSYMperm_null(A, prowscale,pcolscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("DSYMbilu: no symmetric reordering used\n");fflush(stdout);
#endif
	  }
	  /* switch back to C-style */
	  for (i=0; i<n; i++) {
    ...
    */
    if (myblocksize!=NULL) {
	  ...
	  invblock=(integer *)malloc((size_t)n*sizeof(integer));
       ...
       free(idxpos);
#ifdef PRINT_INFO
       mexPrintf("DSYMbilu: companion matrix for symmetric reorderings computed\n");fflush(stdout);
#endif

#ifdef PRINT_INFO2
       mexPrintf("DSYMbilu: call (scaling+) reordering\n");fflush(stdout);
       for (i=0; i<B.nc; i++) {
       ...
       if (!strcmp("mtmetis",options.ordering)) {
	  ierr=DSYMperm_mtmetis(B, prowscale2, pcolscale2, p2, invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("DSYMbilu: mtmetis performed\n");fflush(stdout);
#endif
       }
	  ...
	  ierr=DSYMperm_metis_n(B, prowscale2,pcolscale2, p2,invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("DSYMbilu: metisn performed\n");fflush(stdout);
#endif
       }
       else if (!strcmp("metise",options.ordering)) {
	  ierr=DSYMperm_metis_e(B, prowscale2, pcolscale2, p2, invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("DSYMbilu: metise performed\n");fflush(stdout);
#endif
       }
	  ...
	  ierr=DSYMperm_amd(B, prowscale2, pcolscale2, p2, invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("DSYMbilu: amd performed\n");fflush(stdout);
#endif
       }
       else if (!strcmp("rcm",options.ordering)) {
	  ierr=DSYMperm_rcm(B, prowscale2, pcolscale2, p2, invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("DSYMbilu: rcm performed\n");fflush(stdout);
#endif
       }
	  ...
	  mexPrintf("DSYMbilu: no symmetric reordering used\n");fflush(stdout);
...
#ifdef PRINT_INFO
       mexPrintf("DSYMbilu: block (scaling +) reordering computed, ierr=%d\n",ierr);fflush(stdout);
#endif
#ifdef PRINT_INFO0
       ...
       for (i=0;i<nB; i++) {
	   if (p2[i]<=0 || p2[i]>nB) {
	      mexPrintf("DSYMbilu ERROR, wrong permutation product, step %ld, p2[%ld]=%ld\n", i,i,p2[i]);
	      fflush(stdout);
	   }
	   if (invq2[i]<=0 || invq2[i]>nB) {
	      mexPrintf("DSYMbilu ERROR, wrong permutation product, step %ld, invq2[%ld]=%ld\n", i,i,invq2[i]);
	      fflush(stdout);
	   }
       }
       for (i=0; i<nB; i++)
	   ...
	   if (check_buff[invq2[i]-1]) {
	      mexPrintf("DSYMbilu ERROR, duplicate index, step %ld, invq2[%ld]=%ld\n",i,i,invq2[i]);
	      fflush(stdout);
	   }
	  ...
	  for (j=0; j<mynblocks; j++) {
		 ...
		 mexPrintf("DSYMbilu ERROR, wrong block size, permuted cosine, step %ld, blocksize[%ld]=%ld\n", j,j,myblocksize[j]);
	      if (k>n)
		 mexPrintf("DSYMbilu ERROR, cummulative block size exceeds system size, permuted cosine, step %ld, sum(blocksize)=%ld, n=%ld\n", j,k,n);
	  }  
	  if (k!=n)
	     mexPrintf("DSYMbilu ERROR, cummulative block does not match system size, permuted cosine, nblocks %ld, sum(blocksize)=%ld, n=%ld\n", j,k,n);
#ifdef PRINT_CHECK0
	  ...
	  mexPrintf("\n");fflush(stdout);
	   ...
	   if (invblock[i]<0 || invblock[i]>=n) {
	      mexPrintf("DSYMbilu ERROR, wrong permutation product, step %ld, invblock[%ld]=%ld\n", i,i,invblock[i]);
	      fflush(stdout);
	   }
	   if (invq3[i]<0 || invq3[i]>=n) {
	      mexPrintf("DSYMbilu ERROR, wrong permutation product, step %ld, invq3[%ld]=%ld\n", i,i,invq3[i]);
	      fflush(stdout);
	   }
       }
       for (i=0; i<n; i++)
	   ...
	   if (check_buff[invblock[i]]) {
	      mexPrintf("DSYMbilu ERROR, duplicate index, step %ld, invblock[%ld]=%ld\n",i,i,invblock[i]);
	      fflush(stdout);
	   }
	   ...
	   if (check_buff[invq3[i]]) {
	      mexPrintf("DSYMbilu ERROR, duplicate index, step %ld, invq3[%ld]=%ld\n",i,i,invq3[i]);
	      fflush(stdout);
	   }
...
#ifdef PRINT_INFO
       mexPrintf("DSYMbilu: permutation from reordering of the companion matrix expanded to full size\n");fflush(stdout);
#endif
	   ...
	   if (invq[p[j]]!=j || p[j]<0 || p[j]>=n || invq[j]<0 || invq[j]>=n) {
	      mexPrintf("DSYMbilu ERROR, wrong p/invq after reorering the compressed graph, step %ld, p[%ld]=%ld, invq[%ld]=%ld\n", j,j,p[j],p[j],invq[p[j]]);
	      fflush(stdout);
	   }
       }
       for (i=0; i<n; i++)
...
#ifdef PRINT_INFO
       mexPrintf("DSYMbilu: permutation product built\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
       mexPrintf("DSYMbilu: matrix for BILU prepared\n");fflush(stdout);
#endif
#ifdef PRINT_INFO2
       mexPrintf("p\n");
       for (i=0; i<A.nr; i++) {
    ...
    ia[n]=n;
#ifdef PRINT_INFO
    mexPrintf("DSYMbilu: S_L already exported\n");fflush(stdout);
#endif
    ...
    if (!invert_blocks)
       ...
       pivots=NULL;
    ierr=DSYMbilu(&C, &BL,&BiD,&BL, NULL,NULL, p,invq, &determinant, &isdefinite, 
...
#ifdef PRINT_INFO
    mexPrintf("DSYMbilu: bilu completed, ierr=%d\n",ierr);fflush(stdout);
#endif
    if (options.blocksize==NULL && !ilu1t) {
       if (myblocksize!=NULL)
	  free(myblocksize);
    ...
    if (ierr) {
       mexErrMsgTxt("DSYMbilu: block ILU abnormally terminated") ;
    }
#ifdef _PROFILING_
    ...
    free(p);
#ifdef PRINT_INFO
    mexPrintf("DSYMbilu: P exported\n");fflush(stdout);
#endif

#ifdef PRINT_INFO
    if (options.blocksize==NULL)
       mexPrintf("DSYMbilu: options.blocksize unused\n");
    else
       mexPrintf("DSYMbilu: export options.blocksize\n");
    fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("DSYMbilu: options structure exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("DSYMbilu: BL exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("DSYMbilu: BiD exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("DSYMbilu: memory released\n");fflush(stdout);
#endif
#ifdef PRINT_CHECK
    free(check_buff);
#endif
