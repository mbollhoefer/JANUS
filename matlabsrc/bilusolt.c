/* $Id: bilusolt.c 4153 2018-04-23 19:40:58Z bolle $ */
/* ========================================================================== */
/* === bilusolt mexFunction ============================================== */
/* ========================================================================== */
...
/*
    ...
    z=bilusolt(BL,BiD,BUT,pivots,y)
...
{
    mxArray            *Block, *BlockJ, *BlockI, *BlockE, *BlockD, *pivots_input,
    ...
    integer            i,j,k,l,m,n,*p,isreal,nblocks, *pivots, npivots;
    ...
    if (!mxIsCell(BL_input)) {
       mexErrMsgTxt("bilusolt: first input matrix must be in cell format.") ;
    }
    ...
    if (!mxIsCell(BiD_input)) {
       mexErrMsgTxt("bilusolt: second input matrix must be in cell format.") ;
    }
    
    /* The third input must be a cell array */
    BUT_input=(mxArray *)prhs[2];
    if (!mxIsCell(BUT_input)) {
       mexErrMsgTxt("bilusolt: third input matrix must be in cell format.") ;
    }

    /* The fourth input must be pivots */
    pivots_input=(mxArray *)prhs[3];
    if (!mxIsNumeric(pivots_input))
       mexErrMsgTxt("bilusolt: fourth input argument must be numeric.") ;
    npivots=mxGetM(pivots_input)*mxGetN(pivots_input);
    if (npivots>0) {
       pivots=(integer *)malloc((size_t)npivots*sizeof(integer));
       pr=mxGetPr(pivots_input);
       for (i=0; i<npivots; i++)
	   pivots[i]=pr[i];
    }
    ...
    if (!mxIsNumeric(y_input))
       mexErrMsgTxt("bilusolt: fifth input argument must be numeric.") ;
    n=mxGetM(y_input);
    m=mxGetN(y_input);
...
#ifdef PRINT_INFO
    mexPrintf("bilusolt: check real/non-real\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    if (isreal)
       mexPrintf("bilusolt: real case\n");
    else
       mexPrintf("bilusolt: non-real case\n");
    fflush(stdout);
#endif
    ...
    if (isreal) {
...
#ifdef PRINT_INFO
       mexPrintf("bilusolt: data structures converted\n");
       fflush(stdout);
#endif
...
#ifdef PRINT_INFO
       mexPrintf("bilusolt: call Dbilusolt(%d)\n",++cnt_bilu);
       /*
       for (i=0; i<n; i++)
	 mexPrintf("%8.1le",rhssolr[i]);
       ...
       */
       ...
       Dbilusolt(&BLR,&BiDR,&BUTR,pivots, rhssolr,NULL,n,m);
#ifdef PRINT_INFO
       mexPrintf("bilusolt: Dbilusolt completed\n");
       /*
       for (i=0; i<n; i++)
	 mexPrintf("%8.1le",rhssolr[i]);
       mexPrintf("\n");
       */
       ...
       free(BiDR.valD);
#ifdef PRINT_INFO
    mexPrintf("bilusolt: memory released\n");fflush(stdout);
#endif
    }
    else {
       /* transform data structures and right hand sides */
...
#ifdef PRINT_INFO
       mexPrintf("bilusolt: call Zbilusolt(%d)\n",++cnt_bilu);
       /*
       for (i=0; i<n; i++)
...
#endif
       /* solve triangular system */
       Zbilusolt(&BLC,&BiDC,&BUTC,pivots, rhssolc,NULL,n,m);
#ifdef PRINT_INFO
       mexPrintf("bilusolt: Zbilusolt completed\n");
       fflush(stdout);
#endif
