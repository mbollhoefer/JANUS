/* $Id: ZSYMqmrbilu.c 4155 2018-04-24 11:25:39Z bolle $ */
/* ========================================================================== */
/* ==== ZSYMqmrbilu mexFunction ============================================= */
/* ========================================================================== */
...
/*
    ...
    [sol,flag,iter,resvec]=ZSYMqmrbilu(A,rhs,restol,maxit,BL,BiD,P,SL,pivots,x0);
...

{
    ...
    double     *A_valuesR, *A_valuesI, restol, *pr, *pi, *SL, nrm1inf, locnrm, rval,
               fpar[16], *resvec;
    ...
    if (nrhs!=10)
       mexErrMsgTxt("ZSYMqmrbilu: ten input arguments required.");
    else if (nlhs!=4)
       mexErrMsgTxt("ZSYMqmrbilu: four output arguments are required.");
    ...
    if (!mxIsSparse(A_input))
       mexErrMsgTxt("ZSYMqmrbilu: first input must be a sparse matrix.");
    mrows=mxGetM(A_input);
    ncols=mxGetN(A_input);
    if (mrows!=ncols) {
       mexErrMsgTxt("ZSYMqmrbilu: first input must be a square matrix.");
    }
    ...
    if (!mxIsNumeric(rhs_input))
       mexErrMsgTxt("ZSYMqmrbilu: second input must be a vector.");
    ...
    if (!mxIsNumeric(restol_input))
       mexErrMsgTxt("ZSYMqmrbilu: third input must be a numeric.");
    mrows=mxGetM(restol_input);
    ncols=mxGetN(restol_input);
    if (mrows!=1 || ncols!=1) {
       mexErrMsgTxt("ZSYMqmrbilu: third input must be a scalar.");
    }
    ...
    if (!mxIsNumeric(maxit_input))
       mexErrMsgTxt("ZSYMqmrbilu: fourth input must be a numeric");
    mrows=mxGetM(maxit_input);
    ncols=mxGetN(maxit_input);
    if (mrows!=1 || ncols!=1) {
       mexErrMsgTxt("ZSYMqmrbilu: fourth input must be a scalar.");
    }
    ...
    if (!mxIsCell(BL_input)) {
       mexErrMsgTxt("ZSYMqmrbilu: fifth input matrix must be in cell format.") ;
    }
    ...
    if (!mxIsCell(BiD_input)) {
       mexErrMsgTxt("ZSYMqmrbilu: sixth input matrix must be in cell format.") ;
    }
#ifdef PRINT_INFO
    ...
    if (!mxIsSparse(p_input)) {
       mexErrMsgTxt("ZSYMqmrbilu: seventh input matrix must be a sparse matrix.") ;
    }
    ...
    if (!mxIsSparse(SL_input)) {
       mexErrMsgTxt("ZSYMqmrbilu: eighth input matrix must be a sparse matrix.") ;
    }
    ...
    if (!mxIsNumeric((mxArray *)prhs[8]))
       mexErrMsgTxt("ZSYMqmrbilu: nineth input argument must be numeric.") ;
    ...
    if (!mxIsNumeric(x0_input))
       mexErrMsgTxt("ZSYMqmrbilu: tenth input must be a vector");
	...
	if (!mxIsStruct(Block))
	   mexErrMsgTxt("ZSYMqmrbilu: field BiD{i} must be a structure.");
	   
        /* BiD{i}.J */
	BlockJ=mxGetField(Block,0,"J");
	if (BlockJ==NULL)
	   mexErrMsgTxt("ZSYMqmrbilu: field BiD{i}.J does not exist.");
	if (!mxIsNumeric(BlockJ))
	   mexErrMsgTxt("ZSYMqmrbilu: field BiD{i}.J must be numerical.");
	...
	if (BlockD==NULL)
	   mexErrMsgTxt("ZSYMqmrbilu: field BiD{i}.D does not exist.");
	if (!mxIsNumeric(BlockD))
	   mexErrMsgTxt("ZSYMqmrbilu: field BiD{i}.D must be numerical.");
	/* copy data */
	...
	if (k==1 && !invert_blocks) {
	   rval=sqrt(BiD.valD[i][0].r*BiD.valD[i][0].r+BiD.valD[i][0].i*BiD.valD[i][0].i);
	   BiD.valD[i][0].r= BiD.valD[i][0].r/rval;
	   BiD.valD[i][0].i=-BiD.valD[i][0].i/rval;
	}
	
        /* BL{i} */
	Block=mxGetCell(BL_input,i);
	if (!mxIsStruct(Block))
	   mexErrMsgTxt("ZSYMqmrbilu: field BL{i} must be a structure.");

        /* BL{i}.I */
	BlockI=mxGetField(Block,0,"I");
	if (BlockI==NULL)
	   mexErrMsgTxt("ZSYMqmrbilu: field BL{i}.I does not exist.");
	if (!mxIsNumeric(BlockI))
	   mexErrMsgTxt("ZSYMqmrbilu: field BL{i}.I must be numerical.");
	...
	if (BlockE==NULL)
	   mexErrMsgTxt("ZSYMqmrbilu: field BL{i}.L does not exist.");
	if (!mxIsNumeric(BlockE))
	   mexErrMsgTxt("ZSYMqmrbilu: field BL{i}.L must be numerical.");
	/* copy data */
    ...
    */
    ...
    while (flag) {
	  ...
	  case  6:   
	        ...
	        ZSYMbilusol(&BL,&BiD,&BL,pivots, y,w+ipar[8], n,1);
    ...
    if (ierr<=1)
       ...
       mexErrMsgTxt("ZSYMqmrbilu: undefined SQMR error code");
    ...
    if (ierr) {
       if (ierr==-1)
	  mexPrintf("ZSYMqmrbilu: too many iteration steps\n");
       else if (ierr==-2)
	  mexPrintf("ZSYMqmrbilu: not enough work space\n");
       else if (ierr==-3)
	  mexPrintf("ZSYMqmrbilu: algorithm breaks down\n");
       else 
	  mexPrintf("ZSYMqmrbilu: algorithm stops with error code %ld\n",ierr);
    }
    else { /* success */
       if (ipar[6]>=maxit)
	  mexPrintf("ZSYMqmrbilu: preconditioned SQMR stopped after %ld iteration steps\n", ipar[6]);
       /*
       else
	  mexPrintf("ZSYMqmrbilu: preconditioned SQMR successfully completed with %ld iteration steps\n", ipar[6]);
       */
    }
