...
/*
    ...
    iA=ZHERbfspai(PREC,tol)

    given an approximate block factorization P^T S A S P ~ L D^{-1} L^H  up to
...
)
{
   ...
   mwIndex        *P_ja,          /* row indices of input matrix P         */
...
#ifdef PRINT_INFO
    mexPrintf("ZHERfspai: input parameter tol imported\n");fflush(stdout);
#endif
    ...
    n=*mxGetPr(tmp);
#ifdef PRINT_INFO
    mexPrintf("ZHERbfspai: input parameter PREC.n imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbfspai: input parameter PREC.BL imported\n");fflush(stdout);
#endif
    ZBL.nr=ZBL.nc=n;
    ZBL.nblocks=nblocks;
    ZBL.isreal=0;
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbfspai: PREC.BL prepared\n");fflush(stdout);
#endif
    for (j=0; j<nblocks; j++) {
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbfspai: input parameter PREC.BiD imported\n");fflush(stdout);
#endif
    ZBiD.nr=ZBiD.nc=n;
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbfspai: input parameter PREC.P imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbfspai: input parameter PREC.SL imported\n");fflush(stdout);
#endif

    ZHERbfspai(&iA,&ZBL,&ZBiD,NULL,
	       S_valuesR,NULL,
	       p,invq,mytol);
#ifdef PRINT_INFO
    mexPrintf("ZHERbfspai: symbfspai completed\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbfspai: iA exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbfspai: memory released\n");fflush(stdout);
#endif
