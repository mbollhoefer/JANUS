/* $Id: ZGNLbilu.c 7315 2021-05-28 21:00:20Z bolle $ */
/* ========================================================================== */
/* === ZGNLbilu mexFunction ============================================== */
/* ========================================================================== */
...
/*
    ...
    [BL,BiD,BUT,P,Q,S_L,S_R,determinant,pivots,options]=ZGNLbilu(A,options)
...
)
{
    ...
    integer            i,j,k,l,ll,jj,m,n,*p,*invq,*invq2,*p2,*invq3,*p3,r,s,mynblocks,*myblocksize,flag=0,*pivots,
                       ...
                       level_of_fill=1,
                       cnt, *idx, *idxpos;
    double             *valuesR, *pr, *valuesI, *pi, rd,
    ...
    if (!mxIsSparse(A_input)) {
       mexErrMsgTxt("ZGNLbilu: input matrix must be in sparse format.") ;
    }
    n=mrows;
    ...
    ZGNLAMGinit(&A,&options);
#ifdef PRINT_INFO
    mexPrintf("ZGNLbilu: sparse matrix imported\n");fflush(stdout);
#endif
	...
	if (classIDflags[ifield]==mxCHAR_CLASS) {
	   ...
	   if (!strcmp("matching",fnames[ifield])) {
		 ...
		 invert_blocks=0;
	      ...
	      if (cnt!=n) {
		 mexErrMsgTxt("ZGNLbilu: options.blocksize must exactly match the size of the matrix") ;
	      }
	      cnt=0;
	   }
	   else {
...
#ifdef PRINT_INFO
    mexPrintf("ZGNLbilu: options structure imported\n");fflush(stdout);
#endif
    mxFree(classIDflags);
    mxFree(fnames);
    ...
    rowscale=(doublecomplex *)malloc((size_t)n*sizeof(doublecomplex));
    nB=n;
    ...
    if (!cosine && (options.nblocks==n||options.nblocks==0||options.blocksize==NULL)) {
	  ...
	  to fit ILUPACK requirements */
       ...
       if (options.matching) {
#ifdef PRINT_INFO
	  mexPrintf("ZGNLbilu: use maximum weight matching\n");fflush(stdout);
#endif
	  if (!strcmp("mtmetis",options.ordering)) {
#ifdef _MC64_MATCHING_    
	     ierr=ZGNLperm_mc64_mtmetis(A, rowscale,colscale, p,invq, &nB, &options);
#else
	     ierr=ZGNLperm_matching_mtmetis(A, rowscale,colscale, p,invq, &nB, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: mtmetis performed\n");fflush(stdout);
#endif
	  }
#ifdef _USE_METIS4_
	  else if (!strcmp("metisn",options.ordering)) {
#ifdef _MC64_MATCHING_    
	     ierr=ZGNLperm_mc64_metis_n(A, rowscale,colscale, p,invq, &nB, &options);
#else
	     ierr=ZGNLperm_matching_metis_n(A, rowscale,colscale, p,invq, &nB, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: metisn performed\n");fflush(stdout);
#endif
	  }
	  else if (!strcmp("metise",options.ordering)) {
#ifdef _MC64_MATCHING_    
	     ierr=ZGNLperm_mc64_metis_e(A, rowscale,colscale, p,invq, &nB, &options);
#else
	     ierr=ZGNLperm_matching_metis_e(A, rowscale,colscale, p,invq, &nB, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: metise performed\n");fflush(stdout);
#endif
	  }
#endif // _USE_METIS4_
	  else if (!strcmp("amd",options.ordering)) {
#ifdef _MC64_MATCHING_    
	     ierr=ZGNLperm_mc64_amd(A, rowscale,colscale, p,invq, &nB, &options);
#else
	     ierr=ZGNLperm_matching_amd(A, rowscale,colscale, p,invq, &nB, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: amd performed\n");fflush(stdout);
#endif
	  }
	  else if (!strcmp("rcm",options.ordering)) {
#ifdef _MC64_MATCHING_    
	     ierr=ZGNLperm_mc64_rcm(A, rowscale,colscale, p,invq, &nB, &options);
#else
	     ierr=ZGNLperm_matching_rcm(A, rowscale,colscale, p,invq, &nB, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: rcm performed\n");fflush(stdout);
#endif
	  }
	  else { /* none */
#ifdef _MC64_MATCHING_    
	     ierr=ZGNLperm_mc64_null(A, rowscale,colscale, p,invq, &nB, &options);
#else
	     ierr=ZGNLperm_matching_null(A, rowscale,colscale, p,invq, &nB, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: just scaling performed\n");fflush(stdout);
#endif
	  }
       }
       else { /* no matching */
	  if (!strcmp("mtmetis",options.ordering)) {
	     ierr=ZGNLperm_mtmetis(A, rowscale,colscale, p,invq, &nB, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: mtmetis performed\n");fflush(stdout);
#endif
	  }
	     ...
	     ierr=ZGNLperm_metis_n(A, rowscale,colscale, p,invq, &nB, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: metisn performed\n");fflush(stdout);
#endif
	  }
	  else if (!strcmp("metise",options.ordering)) {
	     ierr=ZGNLperm_metis_e(A, rowscale,colscale, p,invq, &nB, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: metise performed\n");fflush(stdout);
#endif
	  }
	     ...
	     ierr=ZGNLperm_amd(A, rowscale,colscale, p,invq, &nB, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: amd performed\n");fflush(stdout);
#endif
	  }
	  else if (!strcmp("rcm",options.ordering)) {
	     ierr=ZGNLperm_rcm(A, rowscale,colscale, p,invq, &nB, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: rcm performed\n");fflush(stdout);
#endif
	  }
	  else { /* none */
	     ierr=ZGNLperm_null(A, rowscale,colscale, p,invq, &nB, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: just scaling performed\n");fflush(stdout);
#endif
	  }
       ...
       C.val   =(doublecomplex **) malloc((size_t)n*sizeof(double *));
       C.nnz=A.nnz;
       for (i=0; i<n; i++) {
       ...
       ia[n]=n;
#ifdef PRINT_INFO
       mexPrintf("ZGNLbilu: S_L already exported\n");fflush(stdout);
#endif
       ...
       ia[n]=n;
#ifdef PRINT_INFO
       mexPrintf("ZGNLbilu: S_R already exported\n");fflush(stdout);
#endif
	  ...
	  mexErrMsgTxt("ZGNLbilu: block ILU abnormally terminated") ;
       }
...
#ifdef PRINT_INFO
	  mexPrintf("ZGNLbilu: no maximum weight matching, just scaling\n");fflush(stdout);
#endif
	  ierr=ZGNLperm_null(A, rowscale,colscale, p,invq, &nB, &options);
       }
       /* the scalings are real-valued anyway */
...
#ifdef PRINT_INFO
       mexPrintf("ZGNLbilu: scaling and permutation prior to block partioning computed, ierr=%d\n",ierr);fflush(stdout);
#endif
#ifdef PRINT_INFO2
       mexPrintf("rowscale\n");
       for (i=0; i<n; i++) {
	  ...
	  if (cosine) {
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: call modified cosine-based blocking\n");fflush(stdout);
#endif
	     Zmcosine_blocks(&C, p,invq, myblocksize, &mynblocks, TAU);
#ifdef PRINT_INFO
	     mexPrintf("ZGNLbilu: modified cosine-based blocking applied\n");fflush(stdout);
#endif
	  }
	  else {
	     for (i=0; i<n; i++)
...
#ifdef PRINT_INFO
	  mexPrintf("ZGNLbilu: initial permutation turned off\n");fflush(stdout);
#endif
       }
       ...
       free(idxpos);
#ifdef PRINT_INFO
       mexPrintf("ZGNLbilu: companion matrix computed\n");fflush(stdout);
#endif

#ifdef PRINT_INFO2
       mexPrintf("ZGNLbilu: call (scaling+) reordering\n");fflush(stdout);
       for (i=0; i<B.nc; i++){
       ...
       if (!strcmp("mtmetis",options.ordering)) {
	  ierr=ZGNLperm_mtmetis(B, rowscale,colscale, p2,invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZGNLbilu: mtmetis performed\n");fflush(stdout);
#endif
       }
	  ...
	  ierr=ZGNLperm_metis_n(B, rowscale,colscale, p2,invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZGNLbilu: metisn performed\n");fflush(stdout);
#endif
       }
       else if (!strcmp("metise",options.ordering)) {
	  ierr=ZGNLperm_metis_e(B, rowscale,colscale, p2,invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZGNLbilu: metise performed\n");fflush(stdout);
#endif
       }
          ...
          ierr=ZGNLperm_amd(B, rowscale,colscale, p2,invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZGNLbilu: amd performed\n");fflush(stdout);
#endif
       }
       else if (!strcmp("rcm",options.ordering)) {
	  ierr=ZGNLperm_rcm(B, rowscale,colscale, p2,invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZGNLbilu: rcm performed\n");fflush(stdout);
#endif
       }
	  ...
	  for (i=0; i<nB; i++)
	      p2[i]=invq2[i]=i+1;
#ifdef PRINT_INFO
	  mexPrintf("ZGNLbilu: no symmetric reordering used\n");fflush(stdout);
#endif
       }
       free(rowscale);
       free(colscale);
...
#ifdef PRINT_INFO
       mexPrintf("ZGNLbilu: block (scaling +) reordering computed, ierr=%d\n",ierr);fflush(stdout);
#endif
#ifdef PRINT_INFO2   
	  ...
	  memcpy(myblocksize, invblock, mynblocks*sizeof(integer));
       ...
       free(p2);
#ifdef PRINT_INFO
       mexPrintf("ZGNLbilu: permutation expanded\n");fflush(stdout);
#endif
       ...
       free(invq3);
#ifdef PRINT_INFO
       mexPrintf("ZGNLbilu: permutation product built\n");fflush(stdout);
#endif
#ifdef PRINT_INFO2
...
#ifdef PRINT_INFO
       mexPrintf("ZGNLbilu: matrix for BILU prepared\n");fflush(stdout);
#endif
#ifdef PRINT_INFO2
       mexPrintf("p\n");
       for (i=0; i<A.nr; i++) {
	  ...
	  pivots=NULL;
       ierr=ZGNLbilu(&C, &BL,&BiD,&BUT, NULL,NULL, p,invq, &determinant, myblocksize,mynblocks,
		     options.droptol,ilu1t,pa,perturbation,symmetric_structure,pivots,level_of_fill);
       // incorporate diagonal scalings
       determinant.r-=rd;
#ifdef PRINT_INFO
...
#ifdef PRINT_INFO
    mexPrintf("ZGNLbilu: options structure exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZGNLbilu: BL exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZGNLbilu: BUT exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZGNLbilu: memory released\n");fflush(stdout);
#endif
    return;       
