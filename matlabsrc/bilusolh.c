/* $Id: bilusolh.c 4153 2018-04-23 19:40:58Z bolle $ */
/* ========================================================================== */
/* === bilusolh mexFunction ============================================== */
/* ========================================================================== */
...
/*
    ...
    z=bilusolh(BL,BiD,BUT,pivots,y)
...
)
{
    ...
    integer            i,j,k,l,m,n,*p,isreal,nblocks,invert_blocks, *pivots, npivots;
    ...
    if (!mxIsCell(BL_input)) {
       mexErrMsgTxt("bilusolh: first input matrix must be in cell format.") ;
    }
    ...
    if (!mxIsCell(BiD_input)) {
       mexErrMsgTxt("bilusolh: second input matrix must be in cell format.") ;
    }
    
    /* The third input must be a cell array */
    BUT_input=(mxArray *)prhs[2];
    if (!mxIsCell(BUT_input)) {
       mexErrMsgTxt("bilusolh: third input matrix must be in cell format.") ;
    }

    /* The fourth input must be invert_blocks */
    y_input=(mxArray *)prhs[3];
    if (!mxIsNumeric(y_input))
       mexErrMsgTxt("bilusolh: fourth input argument must be numeric.");
    ...
    /* The fiftth input must be the r.h.s. */
    y_input=(mxArray *)prhs[4];
    if (!mxIsNumeric(y_input))
       mexErrMsgTxt("bilusolh: fifth input argument must be numeric.") ;
    n=mxGetM(y_input);
    m=mxGetN(y_input);
...
#ifdef PRINT_INFO
    mexPrintf("bilusolh: check real/non-real\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    if (isreal)
       mexPrintf("bilusolh: real case\n");
    else
       mexPrintf("bilusolh: non-real case\n");
    fflush(stdout);
#endif
    ...
    if (isreal) {
...
#ifdef PRINT_INFO
       mexPrintf("bilusolh: data structures converted\n");
       fflush(stdout);
#endif
...
#ifdef PRINT_INFO
       mexPrintf("bilusolh: call Dbilusolh(%d)\n",++cnt_bilu);
       /*
       for (i=0; i<n; i++)
	 mexPrintf("%8.1le",rhssolr[i]);
       ...
       */
       ...
       Dbilusolh(&BLR,&BiDR,&BUTR,pivots, rhssolr,NULL,n,m);
       if (npivots>0)
	  free(pivots);
#ifdef PRINT_INFO
       mexPrintf("bilusolh: Dbilusolh completed\n");
       /*
       for (i=0; i<n; i++)
	 mexPrintf("%8.1le",rhssolr[i]);
       mexPrintf("\n");
       */
       ...
       free(BiDR.valD);
#ifdef PRINT_INFO
    mexPrintf("bilusolh: memory released\n");fflush(stdout);
#endif
    }
    else {
       /* transform data structures and right hand sides */
...
#ifdef PRINT_INFO
       mexPrintf("bilusolh: call Zbilusolh(%d)\n",++cnt_bilu);
       /*
       for (i=0; i<n; i++)
...
#endif
       /* solve triangular system */
       Zbilusolh(&BLC,&BiDC,&BUTC,pivots, rhssolc,NULL,n,m);
       if (npivots>0)
	  free(pivots);
#ifdef PRINT_INFO
       mexPrintf("bilusolh: Zbilusolh completed\n");
       fflush(stdout);
#endif
