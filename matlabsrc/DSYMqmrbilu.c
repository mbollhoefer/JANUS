/* $Id: DSYMqmrbilu.c 4155 2018-04-24 11:25:39Z bolle $ */
/* ========================================================================== */
/* ==== DSYMqmrbilu mexFunction ============================================= */
/* ========================================================================== */
...
/*
    ...
    [sol,flag,iter,resvec]=DSYMqmrbilu(A,rhs,restol,maxit,BL,BiD,P,SL,pivots,x0);
...

{
    ...
    double     *A_valuesR, restol, *pr, *SL, nrm1inf, locnrm,
    ...
    if (nrhs!=10)
       mexErrMsgTxt("DSYMqmrbilu: ten input arguments required.");
    else if (nlhs!=4)
       mexErrMsgTxt("DSYMqmrbilu: four output arguments are required.");
    ...
    if (!mxIsSparse(A_input))
       mexErrMsgTxt("DSYMqmrbilu: first input must be a sparse matrix.");
    mrows=mxGetM(A_input);
    ncols=mxGetN(A_input);
    if (mrows!=ncols) {
       mexErrMsgTxt("DSYMqmrbilu: first input must be a square matrix.");
    }
    ...
    if (!mxIsNumeric(rhs_input))
       mexErrMsgTxt("DSYMqmrbilu: second input must be a vector.");
    ...
    if (!mxIsNumeric(restol_input))
       mexErrMsgTxt("DSYMqmrbilu: third input must be a numeric.");
    mrows=mxGetM(restol_input);
    ncols=mxGetN(restol_input);
    if (mrows!=1 || ncols!=1) {
       mexErrMsgTxt("DSYMqmrbilu: third input must be a scalar.");
    }
    ...
    if (!mxIsNumeric(maxit_input))
       mexErrMsgTxt("DSYMqmrbilu: fourth input must be a numeric");
    mrows=mxGetM(maxit_input);
    ncols=mxGetN(maxit_input);
    if (mrows!=1 || ncols!=1) {
       mexErrMsgTxt("DSYMqmrbilu: fourth input must be a scalar.");
    }
    ...
    if (!mxIsCell(BL_input)) {
       mexErrMsgTxt("DSYMqmrbilu: fifth input matrix must be in cell format.") ;
    }
    ...
    if (!mxIsCell(BiD_input)) {
       mexErrMsgTxt("DSYMqmrbilu: sixth input matrix must be in cell format.") ;
    }
#ifdef PRINT_INFO
    ...
    if (!mxIsSparse(p_input)) {
       mexErrMsgTxt("DSYMqmrbilu: seventh input matrix must be a sparse matrix.") ;
    }
    ...
    if (!mxIsSparse(SL_input)) {
       mexErrMsgTxt("DSYMqmrbilu: eighth input matrix must be a sparse matrix.") ;
    }
    ...
    if (!mxIsNumeric((mxArray *)prhs[8]))
       mexErrMsgTxt("DSYMqmrbilu: nineth input argument must be numeric.") ;
    ...
    if (!mxIsNumeric(x0_input))
       mexErrMsgTxt("DSYMqmrbilu: tenth input must be a vector");
	...
	if (!mxIsStruct(Block))
	   mexErrMsgTxt("DSYMqmrbilu: field BiD{i} must be a structure.");
	   
        /* BiD{i}.J */
	BlockJ=mxGetField(Block,0,"J");
	if (BlockJ==NULL)
	   mexErrMsgTxt("DSYMqmrbilu: field BiD{i}.J does not exist.");
	if (!mxIsNumeric(BlockJ))
	   mexErrMsgTxt("DSYMqmrbilu: field BiD{i}.J must be numerical.");
	...
	if (BlockD==NULL)
	   mexErrMsgTxt("DSYMqmrbilu: field BiD{i}.D does not exist.");
	if (!mxIsNumeric(BlockD))
	   mexErrMsgTxt("DSYMqmrbilu: field BiD{i}.D must be numerical.");
	...
	if (!mxIsStruct(Block))
	   mexErrMsgTxt("DSYMqmrbilu: field BL{i} must be a structure.");

        /* BL{i}.I */
	BlockI=mxGetField(Block,0,"I");
	if (BlockI==NULL)
	   mexErrMsgTxt("DSYMqmrbilu: field BL{i}.I does not exist.");
	if (!mxIsNumeric(BlockI))
	   mexErrMsgTxt("DSYMqmrbilu: field BL{i}.I must be numerical.");
	...
	if (BlockE==NULL)
	   mexErrMsgTxt("DSYMqmrbilu: field BL{i}.L does not exist.");
	if (!mxIsNumeric(BlockE))
	   mexErrMsgTxt("DSYMqmrbilu: field BL{i}.L must be numerical.");
    ...
    */
    ...
    w  =(double *)malloc((size_t)7*n*sizeof(double));
    y=w+6*n;
    ...
    if (ierr<=1)
       ...
       mexErrMsgTxt("DSYMqmrbilu: undefined SQMR error code");
    ...
    if (ierr) {
       if (ierr==-1)
	  mexPrintf("DSYMqmrbilu: too many iteration steps\n");
       else if (ierr==-2)
	  mexPrintf("DSYMqmrbilu: not enough work space\n");
       else if (ierr==-3)
	  mexPrintf("DSYMqmrbilu: algorithm breaks down\n");
       else 
	  mexPrintf("DSYMqmrbilu: algorithm stops with error code %ld\n",ierr);
    }
    else { /* success */
       if (ipar[6]>=maxit)
	  mexPrintf("DSYMqmrbilu: preconditioned SQMR stopped after %ld iteration steps\n", ipar[6]);
       /*
       else
	  mexPrintf("DSYMqmrbilu: preconditioned SQMR successfully completed with %ld iteration steps\n", ipar[6]);
       */
    }
