/* $Id: ZHERbilu.c 7315 2021-05-28 21:00:20Z bolle $ */
/* ========================================================================== */
/* === ZHERbilu mexFunction ================================================= */
/* ========================================================================== */
...
/*
    ...
    [BL,BiD,P,S_L,determinant,pivots,options]=ZHERbilu(A,options)
...
)
{
  ...
  timeBegin=time_total=omp_get_wtime();
    ...
    if (!mxIsSparse(A_input)) {
       mexErrMsgTxt("ZHERbilu: input matrix must be in sparse format.") ;
    }
    n=mrows;
#ifdef PRINT_CHECK
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbilu: sparse matrix imported\n");fflush(stdout);
#endif
	...
	if (classIDflags[ifield]==mxCHAR_CLASS) {
	   ...
	   if (!strcmp("matching",fnames[ifield])) {
		 ...
		 invert_blocks=0;
	      ...
	      if (cnt!=n) {
		 mexErrMsgTxt("ZHERbilu: options.blocksize must exactly match the size of the matrix") ;
	      }
	      cnt=0;
	   }
	   else {
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbilu: options structure imported\n");fflush(stdout);
    mexPrintf("ZHERbilu: cosine=%ld, ilu1t=%ld, progressive_aggregation=%ld\n",
    ...
    if (options.nblocks==n||options.nblocks==0||options.blocksize==NULL) {
	  ...
	  ierr=ZHERperm_mc64_null(A, rowscale,colscale, p,invq, &nB, &options);
	  ...
	  */
	  ...
	  for (i=0; i<n; i++) {
	      if (p[i]<0 || p[i]>=n) {
		 mexPrintf("ZHERbilu ERROR, wrong permutation, step %ld, p[%ld]=%ld\n",i,i,p[i]);
		 fflush(stdout);
	      }
	      if (invq[i]<0 || invq[i]>=n) {
		 mexPrintf("ZHERbilu ERROR, wrong permutation, step %ld, invq[%ld]=%ld\n",i,i,invq[i]);
		 fflush(stdout);
	      }
	  }
	  for (i=0; i<n; i++)
	  ...
	  for (i=0; i<n; i++) {
	      if (check_buff[p[i]]) {
	         mexPrintf("ZHERbilu ERROR, duplicate index, step %ld, p[%ld]=%ld\n",i,i,p[i]);
		 fflush(stdout);
	      }
	      check_buff[p[i]]=-1;
	  }
	  ...
	  for (i=0; i<n; i++) {
	      if (check_buff[invq[i]]) {
	         mexPrintf("ZHERbilu ERROR, duplicate index, step %ld, invq[%ld]=%ld\n",i,i,invq[i]);
		 fflush(stdout);
	      }
	      check_buff[invq[i]]=-1;
	  }
	  ...
	  for (i=0; i<n; i++) {
	      if (invq[p[i]]!=i) {
	         mexPrintf("ZHERbilu ERROR, inverse permutation does not match, step %ld, p[%ld]=%ld, invq[p[%ld]]=%ld\n",i,i,p[i],i,invq[p[i]]);
		 fflush(stdout);
	      }
	  }
#endif
...
#ifdef PRINT_INFO
	  /* DPrintMatrix(C); */
	  mexPrintf("ZHERbilu: permuted sparse matrix data structures set up\n");fflush(stdout);
#endif
	  ...
	  */
...
#ifdef PRINT_INFO
	  mexPrintf("ZHERbilu: companion matrix for cosine-based algorithm computed\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
	  mexPrintf("ZHERbilu: cosine applied\n");fflush(stdout);
#endif
#ifdef PRINT_CHECK
	  for (i=0; i<m; i++) {
	      if (p2[i]<0 || p2[i]>=m) {
		 mexPrintf("ZHERbilu ERROR, wrong permutation, step %ld, p2[%ld]=%ld\n",i,i,p2[i]);
		 fflush(stdout);
	      }
	  ...
	  for (i=0; i<m; i++) {
	      if (check_buff[p2[i]]) {
	         mexPrintf("ZHERbilu ERROR, duplicate index, step %ld, p2[%ld]=%ld\n",i,i,p2[i]);
		 fflush(stdout);
	      }
	      check_buff[p2[i]]=-1;
	  }
	  ...
	  for (i=0; i<m; i++) {
	      if (invq[p2[i]]!=i) {
	         mexPrintf("ZHERbilu ERROR, inverse permutation does not match, step %ld, p2[%ld]=%ld, invq[p2[%ld]]=%ld\n",i,i,p2[i],i,invq[p2[i]]);
		 fflush(stdout);
	      }
	  }
#endif
	  ...
	  for (i=0;i<n; i++) {
	      if (invq[i]<0 || invq[i]>=n) {
		 mexPrintf("ZHERbilu ERROR, wrong expanded permutation, step %ld, invq[%ld]=%ld\n", i,i,invq[i]);
		 fflush(stdout);
	      }
	  }
	  for (i=0; i<n; i++)
	  ...
	  */
	  ...
	  for (i=0;i<n; i++) {
	      if (p[i]<0 || p[i]>=n) {
		 mexPrintf("ZHERbilu ERROR, wrong permutation product, step %ld, p[%ld]=%ld\n", i,i,p[i]);
		 fflush(stdout);
	      }
	      if (invq[i]<0 || invq[i]>=n) {
		 mexPrintf("ZHERbilu ERROR, wrong permutation product, step %ld, invq[%ld]=%ld\n", i,i,invq[i]);
		 fflush(stdout);
	      }
	  }
	  for (i=0; i<n; i++)
	  ...
	  free(p2);
#ifdef PRINT_INFO
	  mexPrintf("ZHERbilu: permutation after cosine expanded to incorporate 2x2 pivots\n");fflush(stdout);
#endif
#ifdef _PROFILING_
...
#ifdef PRINT_INFO
	  mexPrintf("ZHERbilu: proper sparse matrix data structures based on A set up\n");fflush(stdout);
#endif
#ifdef _PROFILING_
	  time_pre_processing+=omp_get_wtime()-timeBegin;
#endif
	  ...
	  */
	  ...
	  if (!strcmp("mtmetis",options.ordering)) {
	     ...
	     ierr=ZHERperm_mc64_mtmetis(A, rowscale,colscale, p,invq, &n, &options);
#else
	     ierr=ZHERperm_matching_mtmetis(A, rowscale,colscale, p,invq, &n, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("ZHERbilu: matching+mtmetis performed\n");fflush(stdout);
#endif
	  }
#ifdef _USE_METIS4_
	  else if (!strcmp("metisn",options.ordering)) { /* METIS nested dissection by nodes */
#ifdef _MC64_MATCHING_
	     ierr=ZHERperm_mc64_metis_n(A, rowscale,colscale, p,invq, &n, &options);
#else
	     ierr=ZHERperm_matching_metis_n(A, rowscale,colscale, p,invq, &n, &options);
#endif	     
#ifdef PRINT_INFO
	     mexPrintf("ZHERbilu: matching+metisn performed\n");fflush(stdout);
#endif
	  }
	  else if (!strcmp("metise",options.ordering)) {
#ifdef _MC64_MATCHING_
	     ierr=ZHERperm_mc64_metis_e(A, rowscale,colscale, p,invq, &n, &options);
#else
	     ierr=ZHERperm_matching_metis_e(A, rowscale,colscale, p,invq, &n, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("ZHERbilu: matching+metise performed\n");fflush(stdout);
#endif
	  }
#endif // _USE_METIS4_
	  else if (!strcmp("amd",options.ordering)) {
#ifdef _MC64_MATCHING_
	     ierr=ZHERperm_mc64_amd(A, rowscale,colscale, p,invq, &n, &options);
#else
             ierr=ZHERperm_matching_amd(A, rowscale,colscale, p,invq, &n, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("ZHERbilu: matching+amd performed\n");fflush(stdout);
#endif
	  }
	  else {
#ifdef _MC64_MATCHING_
	     ierr=ZHERperm_mc64_rcm(A, rowscale,colscale, p,invq, &n, &options);
#else
	     ierr=ZHERperm_matching_rcm(A, rowscale,colscale, p,invq, &n, &options);
#endif
#ifdef PRINT_INFO
	     mexPrintf("ZHERbilu: matching+rcm performed\n");fflush(stdout);
#endif
	  }
	  /* the scalings are real-valued anyway */
	  for (i=0; i<n; i++) {
	  ...
	  ierr=ZHERperm_null(A, rowscale,colscale, p,invq, &n, &options);
	  /* the scalings are real-valued anyway */
	  for (i=0; i<n; i++) {
	      pcolscale[i]=colscale[i].r;
	  } /* end for */
	  ...
	  */
	  ...
	  if (!strcmp("mtmetis",options.ordering)) {
	     ierr=ZHERperm_mtmetis(A, rowscale,colscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZHERbilu: mtmetis performed\n");fflush(stdout);
#endif
	  }
#ifdef _USE_METIS4_
	  else if (!strcmp("metisn",options.ordering)) {
	     ierr=ZHERperm_metis_n(A, rowscale,colscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZHERbilu: metisn performed\n");fflush(stdout);
#endif
	  }
	  else if (!strcmp("metise",options.ordering)) {
	     ierr=ZHERperm_metis_e(A, rowscale,colscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZHERbilu: metise performed\n");fflush(stdout);
#endif
	  }
#endif // _USE_METIS4_
	  else if (!strcmp("amd",options.ordering)) {
	     ierr=ZHERperm_amd(A, rowscale,colscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZHERbilu: amd performed\n");fflush(stdout);
#endif
	  }
	  else if (!strcmp("rcm",options.ordering)) {
	     ierr=ZHERperm_rcm(A, rowscale,colscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZHERbilu: rcm performed\n");fflush(stdout);
#endif
	  }
	  else { /* none */
	     ierr=ZHERperm_null(A, rowscale,colscale, p,invq, &n, &options);
#ifdef PRINT_INFO
	     mexPrintf("ZHERbilu: no symmetric reordering used\n");fflush(stdout);
#endif
	  }
	  /* the scalings are real-valued anyway */
	  for (i=0; i<n; i++) {
    ...
    */
    if (myblocksize!=NULL) {
	  ...
	  invblock=(integer *)malloc((size_t)n*sizeof(integer));
       ...
       free(idxpos);
#ifdef PRINT_INFO
       mexPrintf("ZHERbilu: companion matrix for symmetric reorderings computed\n");fflush(stdout);
#endif

#ifdef PRINT_INFO2
       mexPrintf("ZHERbilu: call (scaling+) reordering\n");fflush(stdout);
       for (i=0; i<B.nc; i++) {
       ...
       if (!strcmp("mtmetis",options.ordering)) {
	  ierr=ZHERperm_mtmetis(B, rowscale,colscale, p2,invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZHERbilu: mtmetis performed\n");fflush(stdout);
#endif
       }
	  ...
	  ierr=ZHERperm_metis_n(B, rowscale,colscale, p2,invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZHERbilu: metisn performed\n");fflush(stdout);
#endif
       }
       else if (!strcmp("metise",options.ordering)) {
	  ierr=ZHERperm_metis_e(B, rowscale,colscale, p2,invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZHERbilu: metise performed\n");fflush(stdout);
#endif
       }
	  ...
	  ierr=ZHERperm_amd(B, rowscale,colscale, p2,invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZHERbilu: amd performed\n");fflush(stdout);
#endif
       }
       else if (!strcmp("rcm",options.ordering)) {
	  ierr=ZHERperm_rcm(B, rowscale,colscale, p2,invq2, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZHERbilu: rcm performed\n");fflush(stdout);
#endif
       }
	  ...
	  mexPrintf("ZHERbilu: no symmetric reordering used\n");fflush(stdout);
...
#ifdef PRINT_INFO
       mexPrintf("ZHERbilu: block (scaling +) reordering computed, ierr=%d\n",ierr);fflush(stdout);
#endif
#ifdef PRINT_INFO0
       ...
       for (i=0;i<nB; i++) {
	   if (p2[i]<=0 || p2[i]>nB) {
	      mexPrintf("ZHERbilu ERROR, wrong permutation product, step %ld, p2[%ld]=%ld\n", i,i,p2[i]);
	      fflush(stdout);
	   }
	   if (invq2[i]<=0 || invq2[i]>nB) {
	      mexPrintf("ZHERbilu ERROR, wrong permutation product, step %ld, invq2[%ld]=%ld\n", i,i,invq2[i]);
	      fflush(stdout);
	   }
       }
       for (i=0; i<nB; i++)
	   ...
	   if (check_buff[invq2[i]-1]) {
	      mexPrintf("ZHERbilu ERROR, duplicate index, step %ld, invq2[%ld]=%ld\n",i,i,invq2[i]);
	      fflush(stdout);
	   }
	  ...
	  for (j=0; j<mynblocks; j++) {
		 ...
		 mexPrintf("ZHERbilu ERROR, wrong block size, permuted cosine, step %ld, blocksize[%ld]=%ld\n", j,j,myblocksize[j]);
	      if (k>n)
		 mexPrintf("ZHERbilu ERROR, cummulative block size exceeds system size, permuted cosine, step %ld, sum(blocksize)=%ld, n=%ld\n", j,k,n);
	  }  
	  if (k!=n)
	     mexPrintf("ZHERbilu ERROR, cummulative block does not match system size, permuted cosine, nblocks %ld, sum(blocksize)=%ld, n=%ld\n", j,k,n);
#ifdef PRINT_CHECK0
	  ...
	  mexPrintf("\n");fflush(stdout);
	   ...
	   if (invblock[i]<0 || invblock[i]>=n) {
	      mexPrintf("ZHERbilu ERROR, wrong permutation product, step %ld, invblock[%ld]=%ld\n", i,i,invblock[i]);
	      fflush(stdout);
	   }
	   if (invq3[i]<0 || invq3[i]>=n) {
	      mexPrintf("ZHERbilu ERROR, wrong permutation product, step %ld, invq3[%ld]=%ld\n", i,i,invq3[i]);
	      fflush(stdout);
	   }
       }
       for (i=0; i<n; i++)
	   ...
	   if (check_buff[invblock[i]]) {
	      mexPrintf("ZHERbilu ERROR, duplicate index, step %ld, invblock[%ld]=%ld\n",i,i,invblock[i]);
	      fflush(stdout);
	   }
	   ...
	   if (check_buff[invq3[i]]) {
	      mexPrintf("ZHERbilu ERROR, duplicate index, step %ld, invq3[%ld]=%ld\n",i,i,invq3[i]);
	      fflush(stdout);
	   }
...
#ifdef PRINT_INFO
       mexPrintf("ZHERbilu: permutation from reordering of the companion matrix expanded to full size\n");fflush(stdout);
#endif
	   ...
	   if (invq[p[j]]!=j || p[j]<0 || p[j]>=n || invq[j]<0 || invq[j]>=n) {
	      mexPrintf("ZHERbilu ERROR, wrong p/invq after reorering the compressed graph, step %ld, p[%ld]=%ld, invq[%ld]=%ld\n", j,j,p[j],p[j],invq[p[j]]);
	      fflush(stdout);
	   }
       }
       for (i=0; i<n; i++)
...
#ifdef PRINT_INFO
       mexPrintf("ZHERbilu: permutation product built\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
       mexPrintf("ZHERbilu: matrix for BILU prepared\n");fflush(stdout);
#endif
#ifdef PRINT_INFO2
       mexPrintf("p\n");
       for (i=0; i<A.nr; i++) {
    ...
    ia[n]=n;
#ifdef PRINT_INFO
    mexPrintf("ZHERbilu: S_L already exported\n");fflush(stdout);
#endif
    ...
    if (!invert_blocks)
       ...
       pivots=NULL;
    ierr=ZHERbilu(&C, &BL,&BiD,&BL, NULL,NULL, p,invq, &determinant,  &isdefinite, 
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbilu: bilu completed, ierr=%d\n",ierr);fflush(stdout);
#endif
    if (options.blocksize==NULL && !ilu1t) {
       if (myblocksize!=NULL)
	  free(myblocksize);
    ...
    if (ierr) {
       mexErrMsgTxt("ZHERbilu: block ILU abnormally terminated") ;
    }
#ifdef _PROFILING_
    ...
    free(p);
#ifdef PRINT_INFO
    mexPrintf("ZHERbilu: P exported\n");fflush(stdout);
#endif

#ifdef PRINT_INFO
    if (options.blocksize==NULL)
       mexPrintf("ZHERbilu: options.blocksize unused\n");
    else
       mexPrintf("ZHERbilu: export options.blocksize\n");
    fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbilu: options structure exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbilu: BL exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbilu: BiD exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERbilu: memory released\n");fflush(stdout);
#endif
#ifdef PRINT_CHECK
    free(check_buff);
#endif
