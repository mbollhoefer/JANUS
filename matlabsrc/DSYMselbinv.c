...
/*
    ...
    $Id: DSYMselbinv.c 6257 2020-05-13 10:46:34Z bolle $
...
)
{
   ...
   mwIndex        *P_ja,          /* row indices of input matrix P         */
    ...
    n=*mxGetPr(tmp);
#ifdef PRINT_INFO
    mexPrintf("DSYMselbinv: input parameter PREC.n imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("DSYMselbinv: input parameter PREC.BL imported\n");fflush(stdout);
#endif
    DBL.nr=DBL.nc=n;
    DBL.nblocks=nblocks;
    DBL.isreal=1;
...
#ifdef PRINT_INFO
    mexPrintf("DSYMselbinv: PREC.BL prepared\n");fflush(stdout);
#endif
    for (j=0; j<nblocks; j++) {
...
#ifdef PRINT_INFO
    mexPrintf("DSYMselbinv: input parameter PREC.BiD imported\n");fflush(stdout);
#endif
    DBiD.nr=DBiD.nc=n;
...
#ifdef PRINT_INFO
    mexPrintf("DSYMselbinv: input parameter PREC.P imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("DSYMselbinv: input parameter PREC.SL imported\n");fflush(stdout);
#endif

    DSYMselbinv(&iA,&DBL,&DBiD,NULL,
		S_valuesR,NULL,
		p,invq);
#ifdef PRINT_INFO
    mexPrintf("DSYMselbinv: symselbinv completed\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("DSYMselbinv: iA exported\n");fflush(stdout);
#endif
