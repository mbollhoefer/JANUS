...
int main(int argc, char **argv)
{
    ...
    JANUS::SparseMatrix    A;
    JANUS::JanusOptions options;
    JANUS::JanusPrec PREC;
    void *sol, *rhs;
    ...
    //-------------------------------------------------------------------------
    JANUS::JanusDefaultOptions(&options);
    ...
    //---------------------------- compute block ILU --------------------------
    //-------------------------------------------------------------------------
    ierr=JANUS::JanusFactor(&A, &PREC, options);
    //-------------------------------------------------------------------------
    ...
    if (ierr) {
       ...
       JANUS::JanusDelete(&PREC);
       JANUS::SparseDelete(&A);
       free(sol);
       free(rhs);
    ...
    JANUS::JanusNnz(&PREC, &j, &mx, &mu, &stddev);
    if (A.ishermitian || A.issymmetric) {
    ...
    // ------------------------------------------------------------------------
    ierr=JANUS::JanusSolver(&A,&PREC,rhs,sol,K_RESTART,RES_TOL,MAX_IT,&iter);
    // ------------------------------------------------------------------------
...
/*
   ...
   $Id: janusdriver.c 5106 2019-06-19 16:54:15Z bolle $
*/
