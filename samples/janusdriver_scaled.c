...
int main(int argc, char **argv)
{
    ...
    double *pAvalr, *solr, *rhsr;
    doublecomplex *pAvalz, *solz, *rhsz;
    double *SL, *SR;

    // some minor variables
    long int nblocks;
    long int i,j,k,l,m,n,ierr,mx, iter ,flag=0;
    ...
    if (options.blocking_strategy==BLOCK_SUPERNODES) {
	  ...
	  printf("use ILU(1,%8.1le)   blocking\n",DROP_TOL);
       ...
       printf("use ILU(%2d,%8.1le)  blocking\n",LEVEL_OF_FILL,DROP_TOL);
    }
    else // no blocking
       printf(" no blocking\n");
    ...
    if (PREC.ishermitian || PREC.issymmetric)
       printf("log(det(A))=%12.4le+i%12.4le\n",PREC.logdet.r,PREC.logdet.i);
    ...
    if (A.isreal) {
       ...
       for (j=0; j<n; j++) {
	   // rescale A(:,j)
	   pAvalr=(double *)(A.val[j]);
	   for (k=0; k<A.ncol[j]; k++) {
	       // associated row index A(i,j)
	       i=A.rowind[j][k];
	       pAvalr[k]=SL[i]*pAvalr[k]*SR[j];
	   } // end for k
       } // end for j
    }
    else { 
       rhsz=(doublecomplex *)rhs;
       solz=(doublecomplex *)sol;
       // rescale right hand side
       for (i=0; i<n; i++) {
	   rhsz[i].r*=SL[i];
	   rhsz[i].i*=SL[i];
       ...
       for (j=0; j<n; j++) {
	   // rescale A(:,j)
	   pAvalz=(doublecomplex *)(A.val[j]);
	   for (k=0; k<A.ncol[j]; k++) {
	       // associated row index A(i,j)
	       i=A.rowind[j][k];
	       pAvalz[k].r=SL[i]*pAvalz[k].r*SR[j];
	       pAvalz[k].i=SL[i]*pAvalz[k].i*SR[j];
	   } // end for k
       } // end for j
    }
    /*
    ...
    for (i=0; i<n; i++)
	   ...
	   v=MAX(sqrt(rhsz[i].r*rhsz[i].r+rhsz[i].i*rhsz[i].i),v);
    printf("norm rhs: %8.1le\n",v);fflush(stdout);
    */
    // turn off scaling
    PREC.SL=NULL;
    PREC.SR=NULL;
    ...
    for (i=0; i<n; i++)
	   ...
	   v=MAX(sqrt(solz[i].r*solz[i].r+solz[i].i*solz[i].i),v);
    printf("norm sol: %8.1le\n",v);fflush(stdout);
    */
    // re-init scaling
    PREC.SL=SL;
    // be careful in the symmetric case, where only SL exists
    if (!flag)
       PREC.SR=SR;
    if (A.isreal) {
	   ...
	   for (k=0; k<A.ncol[j]; k++) {
	       // associated row index A(i,j)
	       i=A.rowind[j][k];
	       pAvalz[k].r=pAvalz[k].r/(SL[i]*SR[j]);
	       pAvalz[k].i=pAvalz[k].i/(SL[i]*SR[j]);
	   } // end for k
       } // end for j
    }
    // ------------------------------------------------------------------------
    ...
    for (i=0; i<n; i++) {
        if (A.isreal)
	   v=MAX(fabs(solr[i]-1.0),v);
	else
	   v=MAX(sqrt((solz[i].r-1.0)*(solz[i].r-1.0)+solz[i].i*solz[i].i),v);
    }
...
/*
   ...
   $Id: janusdriver.c 3666 2017-09-28 10:47:37Z bolle $
*/
